...
Input: `{item, context, checks, change}` → Output: `{result: pass | fail | needs-human, notes, skipped}`.
`change` is what the gate is being run on — the paths and content of the work since the item entered the active lane — and is what `applies_when` is evaluated against (`FLOW-SPEC.md` § *Declaring what makes a check applicable*). `skipped` lists every check that did not apply, each with the reason, and the caller reports it: a gate that quietly applied to nothing must not look like a gate that passed.
`gate.dod` default: walk the **item's own checklist** first — every unchecked box met with evidence or waived by the user with the reason recorded on the item, and an unmet box holds the gate whatever else passes — then verify the **effective DoD** is satisfied or explicitly N/A'd — judging each declared check against what the project's Definition of Done document (the bus's `## Verification → Definition of Done` row, when present) says it means, and quoting it. The effective DoD is `flow.gates.dod.checks` ∪ `config.dod_gates` — a union, so a project may raise the bar above the flow's baseline and can never silently lower it. (`dod_gates` is a **config** key; `checks` is the flow's.)
...
(see `FLOW-SPEC.md`). `/cadence:doctor` compares that against the **declared
contract version** in `CONTRACT-VERSION.md` — **not** against the plugin version,
which moves for releases no flow author can observe. A flow is stale when the
*contract* moved, not when the plugin did.

That separation arrived at 0.5.0, when a release changed where cadence's config
lives without touching a single hook name, contract or vocabulary term. Under the
old rule every flow — shipped and adopter-authored alike — would have read as stale
after a release that did not change its interface.
