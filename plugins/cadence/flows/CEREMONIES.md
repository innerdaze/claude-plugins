...
> **Not in the next release** (decided 2026-09-08). Ceremonies stay declared and not
> invokable, so a flow listing them gets the defaults below as *documentation of intent*
> rather than behaviour. The one that costs something today is `planning.capture`: without
> it, a sprint flow has no automated way into its committed lane, and a team commits the
> cycle in its tracker by hand. `/cadence:doctor` reports that rather than leaving it to be
> noticed.
...
> to `.agent/cadence/planning-pack-<date>.md` for your meeting, and stops. That
