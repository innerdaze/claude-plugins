...
doc_system:
  kind: none
  notes: .agent/cadence/notes.md       # where record() appends
...
- **`record(note)`** — append the note to `<notes>` (default `.agent/cadence/notes.md`) under a dated bullet, creating the file if absent. This is the minimal fallback so a hard-won gotcha survives even without a doc system.
