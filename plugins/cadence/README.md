...
- **Adapters** — the plugin ships the **contracts** plus zero-dependency fallbacks: `markdown` (tracker), `git` (VCS), `none` (docs), and `none` / `markdown` for the **intent layer** it consumes — when a project has registered stated design, `/cadence:plan` and `/cadence:roadmap` stop on a milestone that contradicts it, and `/cadence:plan` searches existing work before creating any. Adapters for real tools — Linear, GitHub, Jira, whatever you use — are **generated into your project by `/cadence:init`** against the interface that tool actually exposes, because those interfaces vary per install. Cadence assumes no environment.
...
Issues and merge requests: <https://github.com/innerdaze/claude-plugins/issues>.

The contracts — `adapters/ADAPTERS.md`, `flows/HOOKS.md`, `flows/FLOW-SPEC.md` — are treated as public API. Renaming a hook, changing its Input/Output, removing an adapter operation, or changing a config key's meaning is a breaking change, and must be declared in the changelog kept alongside this plugin's design notes in the repository.
