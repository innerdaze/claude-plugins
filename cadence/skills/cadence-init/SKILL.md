...
- **Doc system.** If `domains/` + `INDEX.md` exists → propose `doc_system: domains`, **generate `.claude/cadence/adapters/docs/domains.md`** implementing the doc-system contract against that INDEX (label→doc mapping), and set the config location to `domains/PROJECT.md`. Else if `docs/` → generate a `docs` adapter. Else → the shipped `none` fallback.
...
**B) Author a custom flow** — walk the flow-spec vocabulary (hierarchy, states, gates, cadence/ceremonies, intake/priority policy, decision rights, session definition). For any step the vocabulary can't express, author a **hook doc** (level 3) at the relevant hook from `${CLAUDE_PLUGIN_ROOT}/flows/HOOKS.md`. Write to a project-local `./cadence/<name>.flow.md` (+ `hooks/*.md`).
