...
2. **VCS.** Detect `.git` / `.diversion` / `.jj` / `.hg`. `git` → use the shipped fallback. Anything else → generate `.claude/cadence/adapters/vcs/<kind>.md`.
